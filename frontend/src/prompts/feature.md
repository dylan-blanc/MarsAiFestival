...
Pose ces deux questions en une seule fois :

1. Sur quelle partie de la maquette veux-tu travailler ? _(ex : `header`, `sidebar`, `login form`, `dashboard card`)_
2. Quel nom donnes-tu à cette feature ? _(minuscules, sans espace — ex : `navbar`, `auth`, `schedule`)_

Une fois les réponses reçues, **immédiatement** :

1. Donne la commande Git pour créer la branche :

```bash
git checkout -b feat/[feature-name]
```

2. Analyse la partie ciblée de la maquette :
   - Décris en 2-3 lignes ce que tu vois dans cette zone
   - Liste les composants UI réutilisables → `src/components/ui/`
   - Liste les composants propres à la feature → `src/features/[feature]/components/`
   - Signale tout élément ambigu ou manquant dans la maquette

3. Si l'analyse révèle un besoin d'appel API (formulaire, liste de données...), pose **une seule question ciblée** :
   > _Quel endpoint API est concerné ? (ex : `POST /api/auth` ou "à définir")_

4. Code directement sans attendre de confirmation supplémentaire.
...
- [ ] Les 2 questions de l'étape 1 ont été posées et les réponses reçues
- [ ] La commande `git checkout -b feat/[feature-name]` a été fournie
- [ ] La partie ciblée de la maquette a été analysée avant de coder
